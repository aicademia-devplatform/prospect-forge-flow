...
import { ToolFilterValues } from "./ToolFilters";
import { useTableData } from "@/hooks/useTableData";
import { useUnifiedCRMData, StatusFilters } from "@/hooks/useUnifiedCRMData";
...
import UnifiedCRMFilters from "./UnifiedCRMFilters";
import * as XLSX from "xlsx";
interface TableViewProps {
  tableName: "apollo_contacts" | "crm_contacts" | "hubspot_contacts";
  onBack: () => void;
  isUnifiedView?: boolean; // Nouveau: indique si on utilise la vue unifiée
}
...
// Composant pour afficher les badges de source
const SourceBadges: React.FC<{ row: any }> = ({ row }) => {
  const badges = [];
  ...
  if (row.has_crm) {
    badges.push(
      <Badge
        key="crm"
        variant="secondary"
        className="bg-blue-100 text-blue-700 text-[10px] px-1.5 py-0.5"
      >
        CRM
      </Badge>
    );
  ...
  if (row.has_hubspot) {
    badges.push(
      <Badge
        key="hubspot"
        variant="secondary"
        className="bg-orange-100 text-orange-700 text-[10px] px-1.5 py-0.5"
      >
        HS
      </Badge>
    );
  ...
  if (row.has_apollo) {
    badges.push(
      <Badge
        key="apollo"
        variant="secondary"
        className="bg-purple-100 text-purple-700 text-[10px] px-1.5 py-0.5"
      >
        AP
      </Badge>
    );
...
const TableView: React.FC<TableViewProps> = ({
  ...
  // États spécifiques à la vue unifiée
  ...
  // États pour les filtres d'outils
  const [toolFilters, setToolFilters] = useState<ToolFilterValues>({});

  // État pour contrôler quelle section de filtre est ouverte (exclusif)
  const [openFilterSection, setOpenFilterSection] = useState<
    "none" | "source" | "unified"
  >("none");

  // Déterminer si on doit utiliser la vue unifiée ou la table directe
  // Si des filtres d'outils sont actifs, on bascule sur la table crm_contacts directe
  const hasActiveToolFilters = Object.keys(toolFilters).some((key) => {
  ...
  const getDefaultSortBy = () => {
    if (tableName === "hubspot_contacts") return "inserted_at";
    // Pour la vue unifiée CRM, utiliser last_updated au lieu de created_at
    if (isUnifiedView && tableName === "crm_contacts") return "last_updated";
    return "created_at";
  };
  const [sortBy, setSortBy] = useState(
    urlParams.get("sortBy") || getDefaultSortBy()
  );
  ...
  const hasActiveSourceFilters =
    Object.keys(advancedFilters).some((key) => {
      const value = advancedFilters[key as keyof FilterValues];
      if (key === "dateRange") {
  ...
  const hasActiveUnifiedFilters =
    sourceFilters.length > 0 || Object.keys(statusFilters).length > 0;
  ...
  const [selectedSearchColumns, setSelectedSearchColumns] =
    useState<string[]>(defaultSearchColumns);
  ...
  // Utiliser le hook approprié selon le mode (unifié ou standard)
  // Fusionner advancedFilters et toolFilters avec useMemo pour éviter les re-renders
  const mergedFilters = useMemo(() => {
    const merged = { ...advancedFilters, ...toolFilters };
    console.log("🔍 Filtres fusionnés:", merged);
    console.log("  📊 Tool filters:", toolFilters);
    console.log("  📋 Advanced filters:", advancedFilters);
    return merged;
  }, [advancedFilters, toolFilters]);
  ...
  const standardData = useTableData({
    ...
    sectionFilter:
      selectedSections.length > 0 ? selectedSections.join(",") : "all",
    sortBy,
    sortOrder,
  ...
  const unifiedData = useUnifiedCRMData({
    ...
    statusFilters:
      Object.keys(statusFilters).length > 0 ? statusFilters : undefined,
    advancedFilters: mergedFilters,
  });

  // Sélectionner les données selon le mode
  // Si des filtres d'outils sont actifs, forcer l'utilisation de standardData (table crm_contacts directe)
  const {
    data,
    totalCount,
    totalPages,
  ...
  const generateSectionColor = (sectionName: string) => {
    ...
    const finalName =
      normalizedName === "arlynk" || normalizedName === "arlynk"
        ? "arlynk"
        : normalizedName;
  ...
  useEffect(() => {
    ...
    const toggleableColumns = allColumns
      .filter((col) => col.name !== "email" && col.name !== "id")
      .map((col) => col.name);
    const initialVisible = toggleableColumns.filter((col) =>
  ...
  const pinnedDisplayColumns = allColumns.filter(
    (col) =>
  ...
  const regularDisplayColumns = allColumns
    .filter(
      (col) =>
        !pinnedColumns.has(col.name) && orderedVisibleColumns.includes(col.name)
    )
    .sort(
      (a, b) =>
        orderedVisibleColumns.indexOf(a.name) -
        orderedVisibleColumns.indexOf(b.name)
    );
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const items = Array.from(
      tempColumnOrder.filter((col) => tempVisibleColumns.has(col))
    );
  ...
  const applyColumnChanges = () => {
    ...
    const newOrder = tempColumnOrder.filter((col) =>
      tempVisibleColumns.has(col)
    );
    setTempColumnOrder(newOrder);
  ...
  const saveEdit = async () => {
    ...
    const isEmailField =
      columnName === "email" || columnName.toLowerCase().includes("email");
    if (isEmailField) {
  ...
  const proceedWithSave = async (
    ...
    if (!data || data.length === 0) {
      toast({
        ...
        title: "Erreur",
        description:
          "Les données ne sont pas encore chargées. Veuillez réessayer.",
        duration: 4000,
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${translateColumnName(
          columnName
  ...
  const handleEmailWarningConfirm = async () => {
    if (pendingEmailEdit) {
      ...
      await proceedWithSave(
        pendingEmailEdit.rowId,
        pendingEmailEdit.columnName,
        pendingEmailEdit.value
      );
  ...
  const handleExport = async (options: ExportOptions) => {
    try {
      ...
      const selectedColumns =
        options.columns.length > 0 ? options.columns : Object.keys(contacts[0]);
      ...
      if (options.includeGoogleSheets) {
        toast({
          title: "Google Sheets",
          description:
            "La synchronisation Google Sheets sera disponible prochainement",
          duration: 3000,
        });
      }
    } catch (error) {
  ...
  const exportAsExcel = async (
    ...
    const sheetName =
      tableName === "apollo_contacts" ? "Contacts Apollo" : "Contacts CRM";
  ...
  const exportAsCSV = async (
    ...
    if (includeHeaders) {
      const headers = columns.map((col) =>
        escapeValue(translateColumnName(col))
      );
  ...
  const handleEmailWarningCancel = () => {
    if (pendingEmailEdit) {
      ...
      const originalValue =
        data?.find((row) => row.id === pendingEmailEdit.rowId)?.[
          pendingEmailEdit.columnName
        ] || "";
  ...
  const handleToolFiltersChange = (filters: ToolFilterValues) => {
    setToolFilters(filters);
    setCurrentPage(1);

    // Mapping des filtres d'outils vers leurs colonnes associées (uniquement colonnes existantes)
    const toolColumnMapping: Record<string, string[]> = {
      ...
      brevo_open_number_min: [
        "brevo_tag",
        "brevo_unsuscribe",
        "brevo_open_number",
        "brevo_click_number",
      ],
      brevo_click_number_min: [
        "brevo_tag",
        "brevo_unsuscribe",
        "brevo_open_number",
        "brevo_click_number",
      ...
      apollo_status: ["apollo_status", "apollo_list"],
      apollo_list: ["apollo_status", "apollo_list"],
    };

    // Collecter toutes les colonnes à afficher
    const columnsToShow = new Set<string>();
    Object.keys(filters).forEach((filterKey) => {
      const value = filters[filterKey as keyof ToolFilterValues];
      if (value !== undefined && value !== null && value !== "") {
        const columns = toolColumnMapping[filterKey] || [];
        columns.forEach((col) => columnsToShow.add(col));
      }
    });

    // Ajouter les colonnes au visibleColumns actuel (union, pas remplacement)
    if (columnsToShow.size > 0) {
      setVisibleColumns((prev) => {
        const newSet = new Set(prev);
        columnsToShow.forEach((col) => newSet.add(col));
        return newSet;
      });

      console.log("🎯 Colonnes ajoutées:", Array.from(columnsToShow));
    }
  };

  const handleToolFiltersReset = () => {
    setToolFilters({});
    setCurrentPage(1);
  };
          ...
          <div className="flex flex-wrap gap-1">
            {sections.map((section: string, index: number) => {
              ...
              return (
                <span
                  key={index}
                  className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm ${colorClass}`}
                >
                  {section}
                </span>
              );
    ...
    <div className="h-screen flex flex-col p-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <div className="relative w-64">
              ...
              <DropdownMenu
                open={searchColumnsOpen}
                onOpenChange={setSearchColumnsOpen}
              >
      ...
      <div className="flex items-center justify-between gap-2 mt-5 -mb-0.5 ">
                ...
                return (
                  <Tooltip key={section.value}>
                    ...
                    <TooltipContent>
                      <p>
                        {isSelected
                          ? `Filtrer par ${section.value} (cliquer pour désactiver)`
                          : `Filtrer par ${section.value} (cliquer pour activer)`}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                );
        ...
        <div className="flex items-center gap-2">
            ...
            {(() => {
              ...
              return (
                activeFiltersCount > 0 && (
                  <Badge
                    variant="secondary"
                    className="ml-2 h-5 min-w-5 text-xs flex items-center justify-center"
                  >
                    {activeFiltersCount}
                  </Badge>
                )
          ...
          {/* Bouton pour rafraîchir la vue matérialisée (mode unifié uniquement) */}
          {/* {isUnifiedView && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                    ...
                    onClick={async () => {
                      try {
                        toast({
                          ...
                          description: "La vue unifiée est en cours de mise à jour...",
                        });
                        
                        // Appeler une edge function pour rafraîchir la vue matérialisée
                        const { error } = await supabase.functions.invoke('refresh-unified-view');
                        ...
                        setTimeout(() => {
                          ...
                          toast({
                            ...
                            description: "La vue unifiée a été mise à jour avec succès.",
                          });
                        }, 2000);
                      } catch (error: any) {
                        ...
                        toast({
                          variant: "destructive",
                          title: "Erreur",
                          description: "Impossible de rafraîchir la vue. Tentative de rechargement des données...",
                        });
                        refetch && refetch();
                      }
                    }}
                ...
                <TooltipContent>
                  <p>Rafraîchir la vue matérialisée unifiée avec les dernières données</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )} */}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-9 w-9 p-0 bg-blue-500 text-white hover:bg-blue-600"
      ...
      {filtersOpen && (
        <div className="mt-3 space-y-3">
          {/* Masquer TableFilters si des filtres unifiés sont actifs */}
          {!hasActiveUnifiedFilters && (
            <TableFilters
              ...
              onToolFiltersChange={handleToolFiltersChange}
              onToolFiltersReset={handleToolFiltersReset}
              isFilterExpanded={openFilterSection === "source"}
              onFilterExpandedChange={(expanded) =>
                setOpenFilterSection(expanded ? "source" : "none")
              }
            />
          )}
      ...
      <div
        className={`bg-card rounded-lg border border-border shadow-sm flex-1 flex flex-col min-h-0 mt-3 ${
          filtersOpen ? "min-h-[600px]" : ""
        }`}
      >
        {loading ? (
                  ...
                  <thead className="sticky top-0 bg-table-header border-b border-table-border z-20">
                    <tr>
                      <th className="w-12 px-4 py-4 text-left sticky top-0 left-0 bg-blue-50/95 backdrop-blur-sm border-r border-blue-200/30 z-30">
                        <Checkbox
                          checked={
                            selectedRows.size === (data?.length || 0) &&
                            (data?.length || 0) > 0
                          }
                      ...
                      {displayColumns.map((column) => {
                        ...
                        const isDropdownOpen =
                          openColumnDropdown === column.name;
                        // Since only one column can be pinned, it's always at position 48px (after checkbox)
                        const borderStyle =
                          isScrolled && isPinned
                            ? "border-r-4 border-primary/50 shadow-xl"
                            : isPinned
                            ? "border-r-3 border-primary/40 shadow-lg"
                            : "";
                        return (
                          <th
                            key={column.name}
                            className={`
                               ...
                               ${
                                 isPinned
                                   ? `left-0 z-30 bg-primary/5 backdrop-blur-sm font-semibold text-primary ${borderStyle}`
                            ...
                            <div className="flex items-center justify-between space-x-1">
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="p-2">
                                    <Input
                                      placeholder={`Filtrer ${translateColumnName(
                                        column.name
                                      )}...`}
                      ...
                      <th className="w-20 px-4 py-4 text-center sticky top-0 bg-table-header z-20">
                        <span className="uppercase text-xs tracking-wider font-semibold text-muted-foreground">
                          Actions
                        </span>
                      </th>
                    </tr>
                        ...
                        >
                          ...
                          {displayColumns.map((column) => {
                            ...
                            const wasRecentlyUpdated =
                              recentlyUpdated.has(cellKey);
                            return (
                              <td
                                key={column.name}
                                className={`px-4 py-4 min-w-[120px] ${
                                  isPinned
                                    ? `sticky bg-primary/5 backdrop-blur-sm z-10 font-semibold text-primary ${borderStyle}`
                                    : ""
                                } ${
                                  canEdit
                                  ...
                                  wasRecentlyUpdated
                                    ? "bg-green-50 border-l-4 border-green-400 shadow-sm animate-pulse"
                                    : ""
                                }`}
                                style={
                                  isPinned
                                  ...
                                  <div className="flex items-center gap-2">
                                    {column.name === "data_section" ? (
                                      ...
                                      >
                                        ...
                                        <SelectContent>
                                          <SelectItem value="Arlynk">
                                            Arlynk
                                          </SelectItem>
                                          <SelectItem value="Aicademia">
                                            Aicademia
                                          </SelectItem>
                                        </SelectContent>
                                      </Select>
                            ...
                            <div className="flex items-center justify-center space-x-1">
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  // En mode unifié, utiliser l'email pour naviguer au lieu de l'ID
                                  if (isUnifiedView && row.email) {
                                    currentParams.set("email", row.email);
                                    navigate(
                                      `/contact/${tableName}/by-email?${currentParams.toString()}`
                                    );
                                  } else {
                                    navigate(
                                      `/contact/${tableName}/${rowId}?${currentParams.toString()}`
                                    );
                                  }
                                }}
                              >
                              ...
                              <Button
                                ...
                                disabled={!hasPermission("delete_prospects")}
                              >
      ...
      <Dialog open={columnDialogOpen} onOpenChange={setColumnDialogOpen}>
        <DialogContent className="max-w-5xl w-[90vw] h-[80vh] bg-background flex flex-col">
          <DialogHeader className="flex-shrink-0">
            ...
            <DialogDescription>
              Glissez et déposez les colonnes pour réorganiser leur ordre
              d'affichage. Déplacez les colonnes entre les sections pour les
                                ...
                                {(provided, snapshot) => (
                                  <div
                                    ...
                                    className={`flex items-center gap-2 p-2 bg-background rounded border border-green-200 hover:border-green-300 transition-all duration-200 ${
                                      snapshot.isDragging
                                        ? "shadow-lg scale-105 rotate-1 bg-green-50 z-50"
                                        : "hover:shadow-md"
                                    }`}
                                    style={{
                ...
                Colonnes masquées (
                {
                  allColumns.filter(
                    (col) =>
                      col.name !== "email" &&
                      col.name !== "id" &&
                      !tempVisibleColumns.has(col.name)
                  ).length
                }
                )
              </h3>
            ...
            <AlertDialogDescription className="space-y-3">
              ...
              {contactToDelete && (
                <div className="bg-muted p-3 rounded-lg border-l-4 border-destructive">
                  <p>
                    <strong>Nom :</strong> {contactToDelete.name}
                  </p>
                  <p>
                    <strong>Email :</strong> {contactToDelete.email}
                  </p>
                  <p>
                    <strong>ID :</strong> {contactToDelete.id}
                  </p>
                </div>
              )}
              <p className="text-destructive font-medium">
                ⚠️ Cette action est <u>irréversible</u>. Toutes les données
                liées à ce contact seront perdues définitivement.
              </p>
              <p>Êtes-vous absolument certain(e) de vouloir continuer ?</p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
