...
import { Download, FileSpreadsheet, Check, Settings, Columns, FileType } from 'lucide-react';
...
export interface ColumnDefinition {
  key: string;
  label: string;
  category?: 'basic' | 'contact' | 'company' | 'status' | 'custom';
  enabled?: boolean;
}
...
const defaultCrmColumns: ColumnDefinition[] = [
  ...
  { key: 'linkedin_url', label: 'LinkedIn', category: 'contact', enabled: false },
  { key: 'city', label: 'Ville', category: 'company', enabled: false },
  { key: 'country', label: 'Pays', category: 'company', enabled: false },
  { key: 'zoho_status', label: 'Statut Zoho', category: 'status', enabled: false },
  { key: 'apollo_status', label: 'Statut Apollo', category: 'status', enabled: false },
  { key: 'contact_active', label: 'Contact actif', category: 'status', enabled: false },
  { key: 'industrie', label: 'Industrie', category: 'company', enabled: false },
  { key: 'created_at', label: 'Date de création', category: 'custom', enabled: false },
  { key: 'updated_at', label: 'Date de mise à jour', category: 'custom', enabled: false },
];
...
const defaultApolloColumns: ColumnDefinition[] = [
  ...
  { key: 'work_direct_phone', label: 'Téléphone direct', category: 'contact', enabled: false },
  { key: 'mobile_phone', label: 'Mobile', category: 'contact', enabled: false },
  { key: 'person_linkedin_url', label: 'LinkedIn profil', category: 'contact', enabled: false },
  { key: 'company_linkedin_url', label: 'LinkedIn entreprise', category: 'company', enabled: false },
  { key: 'website', label: 'Site web', category: 'company', enabled: false },
  { key: 'industry', label: 'Industrie', category: 'company', enabled: false },
  { key: 'nb_employees', label: 'Nombre d\'employés', category: 'company', enabled: false },
  { key: 'seniority', label: 'Séniorité', category: 'status', enabled: false },
  { key: 'email_status', label: 'Statut email', category: 'status', enabled: false },
  { key: 'created_at', label: 'Date de création', category: 'custom', enabled: false },
];
...
const ExportDialog: React.FC<ExportDialogProps> = ({
  ...
  const [csvEncoding, setCsvEncoding] = useState<'UTF-8' | 'ISO-8859-1' | 'Windows-1252'>('UTF-8');
  const [includeHeaders, setIncludeHeaders] = useState(true);
  const [quoteStrings, setQuoteStrings] = useState(true);

  // Columns management
  const defaultColumns = tableName === 'crm_contacts' ? defaultCrmColumns : defaultApolloColumns;
          ...
          <ScrollArea className="flex-1 pr-4">
                ...
                <RadioGroup value={scope} onValueChange={(value) => setScope(value as 'current' | 'all')}>
                  <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent cursor-pointer">
                    ...
                    <Label htmlFor="current" className="cursor-pointer flex-1">
                      <div className="font-medium">Page actuelle seulement</div>
                      <div className="text-xs text-muted-foreground">{currentPageCount} contacts visibles</div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent cursor-pointer">
                    <RadioGroupItem value="all" id="all" />
                    <Label htmlFor="all" className="cursor-pointer flex-1">
                      <div className="font-medium">Toutes les pages avec filtres</div>
                      <div className="text-xs text-muted-foreground">{totalCount} contacts au total</div>
                    </Label>
                  </div>
                </RadioGroup>
              </div>
            ...
            <TabsContent value="columns" className="space-y-4 mt-4">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">
                  Sélection des colonnes ({selectedColumns.length} sélectionnées)
                </Label>
                <div className="flex gap-2">
                  <Button 
                    variant="outline" 
              ...
              {Object.entries(columnsByCategory).map(([category, categoryColumns]) => {
                ...
                const allEnabled = categoryColumns.every(col => col.enabled);
                const someEnabled = categoryColumns.some(col => col.enabled);
                      ...
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-sm flex items-center gap-2">
                          ...
                          <Badge variant="secondary" className="text-xs">
                            {categoryColumns.filter(col => col.enabled).length}/{categoryColumns.length}
                          </Badge>
                        </CardTitle>
                        <Checkbox
                          checked={allEnabled}
                          onCheckedChange={(checked) => toggleCategoryColumns(category, checked === true)}
                          className={someEnabled && !allEnabled ? "data-[state=checked]:bg-orange-600" : ""}
                        />
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      {categoryColumns.map(col => (
                        <div key={col.key} className="flex items-center space-x-2 p-2 hover:bg-accent rounded">
                          <Checkbox
                            id={`col-${col.key}`}
                            checked={col.enabled}
            ...
            <TabsContent value="format" className="space-y-6 mt-4">
              ...
              <div className="space-y-3">
                ...
                <RadioGroup value={format} onValueChange={(value) => setFormat(value as 'xlsx' | 'csv' | 'json')}>
                  <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent cursor-pointer">
                ...
                <>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="delimiter" className="text-sm">Délimiteur</Label>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="encoding" className="text-sm">Encodage</Label>
                        <Select value={csvEncoding} onValueChange={(value) => setCsvEncoding(value as any)}>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => setQuoteStrings(checked === true)}
                          />
