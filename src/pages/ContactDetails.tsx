...
const ContactDetails: React.FC = () => {
  ...
  const personalFields = tableName === 'apollo_contacts' 
    ? ['first_name', 'last_name', 'email', 'title', 'seniority', 'person_linkedin_url']
    : ['firstname', 'name', 'full_name', 'email', 'linkedin_function', 'linkedin_url'];

  const companyFields = tableName === 'apollo_contacts'
    ? ['company', 'company_linkedin_url', 'website', 'industry', 'company_phone', 'company_address', 'company_city', 'company_country']
    : ['company', 'company_website', 'linkedin_company_url', 'industrie', 'nb_employees'];

  const contactFields = tableName === 'apollo_contacts'
    ? ['work_direct_phone', 'mobile_phone', 'home_phone', 'corporate_phone', 'other_phone']
    : ['tel', 'tel_pro', 'mobile', 'mobile_2'];

  const statusFields = tableName === 'apollo_contacts'
    ? ['statut', 'lifecycle_stage', 'stage', 'email_status', 'email_confidence']
    : ['data_section', 'contact_active', 'zoho_status', 'apollo_status', 'brevo_status', 'hubspot_status'];
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {renderFieldGroup('Informations personnelles', personalFields, <User className="h-5 w-5" />)}
          {renderFieldGroup('Entreprise', companyFields, <Building className="h-5 w-5" />)}
          {renderFieldGroup('Contact', contactFields, <Phone className="h-5 w-5" />)}
          {renderFieldGroup('Statut & Suivi', statusFields, <Tag className="h-5 w-5" />)}
        </div>
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {['created_at', 'updated_at', 'last_sync_at', 'last_contacted'].map(field => {
                const value = contact[field];
